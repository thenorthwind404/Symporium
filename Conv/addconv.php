...
<form action='./addconv.php' method='POST'>
<lable>Choose Convenor to be Added : </lable> <br><br>
<select name ="convenor2">
	<?php
		$result=mysqli_query($con,"SELECT conv_id, UserName FROM users WHERE conv_id IS NOT NULL AND conv_id <> $mainc");
		while($r=mysqli_fetch_assoc($result))
		{
			$c=$r['conv_id'];
			echo "<option value='$c'>".$r['UserName']."</option>";	
		}
	?>
</select>
<br>
...
{	
	$cid=$_SESSION['cid'];
	$c=$_POST['convenor2'];
	mysqli_query($con,"UPDATE `conf` SET `heldby2`='$c' WHERE conferenceid = '$cid'");
	echo "<script>alert('New Convenor has been added successfully');</script>";	
}
?>