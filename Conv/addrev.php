  ...
  <a href="#" id="con">Terms and Conditions</a>
    ...
    <div class = "form_inner_design">	
		<?php
			if(isset($_POST['rev']))
			{
				$cname=$_POST['confname'];
				$cid=$_POST['confid'];
				$rid=$_POST['revid'];
			}			
			echo "<h1><center>Available Reviewers</center></h1><br>";
			echo "<h1><center>Conference ID ".$cid."</center></h1><br>"; 
			echo "<h1><center>Conference Name ".$cname."</center></h1><br>"; 
		?>
            <center><table>

                <col width = "300">
                    <col width = "1000">	
		<tr><th>Reviewer ID</th><th> Reviewer Name</th></tr>
	<?php
		$result=mysqli_query($con,"SELECT rev_id, UserName FROM users WHERE rev_id IS NOT NULL ");
		while($r=mysqli_fetch_assoc($result))
			echo "<tr><td style='text-align:center'>".$r['rev_id']."</td><td style='text-align:center'>".$r['UserName']."</td></tr>"; 
	?>
	</table></center><br><br>
        <form name="myform" action="addrev.php" method="POST" enctype="multipart/form-data">
		            <table>
                <col width = "700">
                    <col width = "700">
                    <tr>
                    ...
                    <td>       
                       <input type = 'number' name = 'confid' required>
                    </td>
                </tr>
                    <tr>
                    <td class = 'bold'>
                    ...
                    <td>       
                       <input type = 'text' name = 'confname' required>
                    </td>
                </tr>
                    <tr>
                    <td class = 'bold'>
                        Reviewer ID to be Added 
                    </td>
                    <td>       
...
if(isset($_POST['rev']))
{
	...
	$rid=$_POST['revid'];
	$rname=$_POST['revname'];
	$result3=mysqli_query($con,"SELECT `confid`, `revid`, `revname` FROM `reviewers` WHERE confid=$cid AND revid=$rid AND revname='$rname'");
	if(mysqli_num_rows($result3)==0)
	{
	$sql1 = "INSERT INTO `reviewers`(`confid`, `revid`,`revname`, `total`) VALUES ($cid,$rid,'$rname',0)";
	mysqli_query($con,$sql1);
	$result4=mysqli_query($con,"SELECT Email FROM users WHERE rev_id=$rid");
	$r4=mysqli_fetch_assoc($result4);
	$email=$r4['Email'];
	$head = "From:harshithbn1808@gmail.com \r\n";
	$msg="Hello ".$rname." you have been added as reviewer in a Conference. Conference Name - ".$cname." Conference ID - ".$cid.". Please Login to review the assigned Papers." ; 
	mail($email,"Added To New Conference!!",$msg,$head);
	echo "<script>alert('Reviewer has been successfully added. Email has been Sent.');</script>";
	}	
	else
	echo "<script>alert('Reviewer already added perviously');</script>";	
}
?>