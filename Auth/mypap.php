  ...
  <a href="#" id="con">Terms and Conditions</a>
            ...
            <div class="mainbox">
                <?php 
                  ...
                  $x = $_SESSION['auth_id'];
                  $query = "SELECT * from files WHERE campap IS NULL AND auth_id = $x AND fullpaper IS NOT NULL AND r_rate > -1 ";
                  $result = mysqli_query($db,$query);
                  echo "<div class='flex-container'>";
                  if (mysqli_num_rows($result) > 0)
                  {
                    echo "<div>Camera Ready Papers</div>";
                    //echo "<div class='flex-container'>";
                    while($row = $result->fetch_assoc()) 
                    {
                      ...
                      echo "<h1 id = 'details'>Camera-Ready Paper Submission</h1>";
                      echo "<br/>".$res1['conf']."<br/><br/>";
                      echo "".$res1['topic']."<br/><br/>";
                      $d2 = $res1['confdate'];
                      echo "Due Date  : ".$d2."<br/>";
                      if($row['r_rate']>=0){echo "<a id = 'buttonid' href = './mypap2.php?confid=$y'>Submit Camera Ready Paper</a>";}
                      ...
                      <tr>
                        ...
                        <td>
                          <?php
                          if($row['r1_comments'] == NULL)
                          {
                            echo "No comments from Reviewer 1 yet!";
                          } 
                          else 
                          {
                            echo $row['r1_comments'];
                          }
                          ?>
                        </td>
                        </tr>
                        ...
                        <td>
                          <?php
                          if($row['r2_comments'] == NULL)
                          {
                            echo "No comments from Reviewer 2 yet!";
                          } 
                          else 
                          {
                            echo $row['r2_comments'];
                          }
                          ?>
                        </td>
                        </tr>
                        ...
                        <td>
                          ...
                          else if($row['r_rate'] > 0) 
                          {
                            echo "Paper has been accepted. Please submit Camera Ready Paper";
                          }
                          else if($row['r_rate'] <= 0) 
                          {
                            echo "Paper has been rejected";
                          }
                          ?>
                        </td>
                        </tr>
                  ...
                  {
                    echo "<div>No Camera Ready Papers to submit</div>";
                  }
                  echo "</div>";
                  ?> 
                  <?php
                  $q3 = "SELECT * from files WHERE auth_id = $x";
                  $ress = mysqli_query($db,$q3);
                  echo "<div class='flex-container'>";
                  if (mysqli_num_rows($ress) > 0)
                  {
                    echo "<div>Video Conferences</div>";
                    while($row = $ress->fetch_assoc())
                    {
                      echo "<div>";
                      $cid = $row['conf_id'];
                      $q4 = "SELECT * from conf WHERE conferenceid = $cid";
                      $ress2 = mysqli_query($db,$q4);
                      $ress3 = mysqli_fetch_assoc($ress2);
                      if($ress3['conflink'] != NULL)
                      {
                        echo "<h1 id = 'details'>".$ress3['conf']."</h1><br/>";
                        echo $ress3['topic']."</br><br/>";
                        echo "Conference date : ".$ress3['confdate'];
                        $s1 = $ress3['conflink'];
                        echo "<a href = '$s1' target='_blank' id = 'buttonid'>Video Conference link</a> ";
                        if($ress3['confpass'] != NULL)
                        {
                          echo "<br/><br/>Conference pass : ".$ress3['confpass'];
                        }
                        echo "</div>";
                      }
                      echo "</div>";
                  ...
                  {
                    echo "<div>No Video Conferences</div>";
                  }
