...
{ // if save button on the form is clicked
    ...
    } else 
    {
        ...
        {
            $sql = "UPDATE files SET campap = '$filename' WHERE conf_id = $confid AND auth_id = $authid";
            $sql1 = "UPDATE files SET camsize = $size WHERE conf_id = $confid AND auth_id = $authid";
            ...
            {
                echo "<script type='text/javascript'>alert('Failed to update table!'); window.location.replace('./mypap2.php')</script>";
            }
        } 
        else 
        {
            echo "<script type='text/javascript'>alert('Failed to upload file');  window.location.replace('./mypap2.php')</script>";
        }
    }
